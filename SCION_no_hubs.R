...
SCION_no_hubs <- function(target_genes_file,reg_genes_file,target_data_file,reg_data_file,clustering_data_file){
  ...
  clusterresults <- read.csv(clustering_data_file,stringsAsFactors=FALSE)
  
  #infer a network using GENIE3 on each cluster
  for (i in 1:max(clusterresults$Cluster)){
    mygenes = clusterresults[clusterresults$Cluster==i,1]
    ...
    #if there are no targets, we also skip the cluster
    if (dim(clustertargetdata)[1]<1){
      next
    }
    
    network = RS.Get.Weight.Matrix_cluster(t(clustertargetdata),t(clusterregdata))
  ...
  #   network = RS.Get.Weight.Matrix_cluster(t(hubtargetdata),t(hubregdata))
  #   
  #   #trim matrix based on the ratio of TFs to targets - more TFs per target = more edges kept
