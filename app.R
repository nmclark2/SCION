...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(h3("Files"),
      ...
      numericInput('weight', label = "Edge Weight Cutoff", min=0, max=1, value = 0.33, step=0.1),
    ...
    mainPanel(h3("Input files"),
              ...
              p("Cutoff to use for trimming the network. A cutoff of 0 keeps all inferred edges. The closer the cutoff is to 1, the less edges are kept. Higher cutoff = more confidence in the inferred edges. We recommend using some sort of cutoff except in the case of very small networks. Default: 0.33"),
              strong("Clustering"),
              ...
              p("Indicate the clustering threshold to use. The clustering threshold is how similar the
                ...
                p("For Temporal Clustering: Range 0 to 1. If you pick a threshold close to 1, you will
                ...
                p("For Non-Temporal Clustering: Range 0 to 5. If you pick a threshold close to 5, you will obtain few clusters with low within-cluster similarity. If you pick a threshold close to 0, you will obtain many clusters with high within-cluster similarity."),
  ...
  observe({
    if (input$action > 0){
      SCION(target_genes_file=input$file3$datapath,reg_genes_file=input$file4$datapath,
            target_data_file=input$file$datapath,reg_data_file=input$file2$datapath, weightthreshold=input$weight,
