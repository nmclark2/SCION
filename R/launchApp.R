...
#' Requires `shiny`, `shinydashboard`, `shinydashboardPlus`, and `shinyjs`,
#' which are `Suggests` (not hard dependencies) so that scripted/HPC use of
#' the package doesn't require the Shiny stack to be installed.
#'
#' @param ... additional arguments passed to `shiny::shinyApp()`.
#' @return a `shiny.appobj`, as returned by `shiny::shinyApp()`.
#' @export
launchApp <- function(...) {
  required <- c("shiny", "shinydashboard", "shinydashboardPlus", "shinyjs")
